...
```{r supply-solution, warning = FALSE, message=FALSE}
library(ggplot2)
ggplot() + 
...
```{r marketSupplyPlot-solution, message=FALSE, warning=FALSE}
library(ggplot2)
ggplot() + 
  geom_function(
...
```{r equilibrium-setup, message=FALSE, warning=FALSE}
library(ggplot2)
```

```{r equilibrium-solution, message=FALSE, warning=FALSE}
library(ggplot2)
ggplot() + 
  geom_function(
    fun = function(x) x ^ 2 / 6.48e12 - x / 1.44e6 + 1, 
...
```{r eqSolve-setup, message=FALSE, warning=FALSE}

```

```{r eqSolve-solution, message=FALSE, warning=FALSE}
library(nleqslv)
srEq <- nleqslv(
  c(1,4.5e6), 
  function(x) c(
    x[1]*x[2] - 4.5e6,
    x[2] ^ 2 / 6.48e12 - x[2] / 1.44e6 + 1 - x[1]
    )
  )
srEq$x
```
