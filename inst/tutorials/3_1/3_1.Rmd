...
bc <- function(x) p1[i]*x[1] + p2*x[2] # p1 is a vector to loop through the different prices.
c1 <- NULL
...
```{r identicalConsumers, exercise = TRUE}
p1 <- seq(0.25, 2.5, 0.05)
bc <- function(x) p1[i]*x[1] + p2*x[2] # p1 is a vector to loop through prices.
c1.1 <- NULL
c1.2 <- NULL
c1.3 <- NULL
for (i in 1:length(p1)) {
  ...
  c1.1[i] <- c[1]
  c1.2[i] <- 2 * c[1] # With 2 identical consumers, demand is just 2x that of 1.  
  c1.3[i] <- 3 * c[1]
}
plot(
  x = c1.3,
...
```{r differentConsumersSetup, exercise = TRUE}
set.seed(8675309) # Set the random "seed" so we all get the exact same results
p1 <- seq(0.25, 2.5, 0.05)
a <- runif(200, min = 0.1, max = 0.9) # Sequence of random budget shares
m <- rlnorm(200, meanlog = log(100), sdlog = 0.25) # Sequence of random incomes
...
```{r differentConsumersEquilibrium, exercise = TRUE}
for (i in 1:length(p1)) {
  if (i == 1) {
    pars <- c(1, 1)
  } else {
...
```{r differentConsumersEquilibrium-setup, exercise = TRUE}
library(Rsolnp)
...
```{r differentConsumersPlots-setup, exercise = TRUE}
library(Rsolnp)
set.seed(8675309); m <- rlnorm(200, meanlog = log(100), sdlog = 0.25) 
p1 <- seq(0.25, 2.5, 0.05); p2 = 1; a <- runif(200, min = 0.1, max = 0.9)
c1 <- matrix(nrow = length(p1), ncol = length(a)) 
...
bc <- function(x) p1[i] * x[1] + p2 * x[2]
for (i in 1:length(p1)) {
  if (i == 1) {pars <- c(1, 1)} else {pars <- c$pars}
  for (j in 1:length(m)) {
    c <- solnp(pars = pars, ux, ineqfun = bc, ineqLB = 0, ineqUB = m[j],
...
```{r marketEquilibrium, exercise = TRUE}

```

```{r marketEquilibrium-setup, exercise = TRUE}
library(Rsolnp)
set.seed(8675309); m <- rlnorm(200, meanlog = log(100), sdlog = 0.25) 
p1 <- seq(0.25, 2.5, 0.05); p2 = 1; a <- runif(200, min = 0.1, max = 0.9)
c1 <- matrix(nrow = length(p1), ncol = length(a)) 
