...
```{r initialEquilibrium, echo = TRUE, results=FALSE}
ux <- function(x)
  - x[1] ^ (a) * x[2] ^ (1 - a)
bc <- function(x)
  p1 * x[1] + p2 * x[2]
...
```{r cdIncomeChange, exercise = TRUE, exercise.setup = 'initialEquilibrium'}
...
```{r dmEquilibrium, exercise.setup = 'initialEquilibrium'}
m = 100
dm = 5
...
```{r priceChange, exercise = TRUE, exercise.setup = 'initialEquilibrium'}
...
```{r dpEquilibrium, exercise.setup = 'initialEquilibrium'}
dp1 = 0.5
bcLowP  <- function(x)
  (p1 - dp1) * x[1] + p2 * x[2]
bcHighP <- function(x)
...
```{r analyticalStatics, exercise = TRUE, exercise.setup = 'demand-solution'}
...
```{r cdEngel, exercise = TRUE, exercise.setup = 'dmEquilibrium'}
...
```{r nonlinearIEP, exercise = TRUE, exercise.setup = 'initialEquilibrium'}
m4 = seq(60, 120, 20)
...
```{r demandPoints, exercise = TRUE, exercise.setup = 'dpEquilibrium'}
...
```{r demandSegments, exercise = TRUE, exercise.setup = 'dpEquilibrium'}
plot(
  initialEquilibrium$pars[1],
  p1,
  type = 'p',
...
```{r demandCurve, exercise = TRUE, exercise.setup = 'dpEquilibrium'}
...
```{r demandFunction, exercise = TRUE, exercise.setup = 'dpEquilibrium'}
...
```{r xTilde, exercise = TRUE, exercise.setup = 'dpEquilibrium'}
library(Rsolnp)
...
Once you've found $\tilde{x}$, finding the consumer surplus is relatively straightforward. In our case, the expenditures needed at the new prices in order to reach the original utility is the value of the expenditure function at $\tilde{x}$, which equals the last value in `xTilde$values`, or `r round(tail(xTilde$values, 1), 2)`; the expenditures needed to reach the new optimal utility is simply the consumer's money income, `m`. The change in consumer surplus will be the difference `r round(m - tail(xTilde$values, 1), 2)`. 
