...
library(Rsolnp); a = 0.9; m = 20; pb = 2; pd = 1; t = 0.5; sd = 5
ux <- function(x) - x[1] ^ (a) * x[2] ^ (1 - a)
bcT <- function(x) (pb + t) * x[1] + pd * x[2]
bc <- function(x) pb * x[1] + pd * x[2]
eqBarney0 <- solnp(c(1, 1), ux, ineqfun = bc, ineqLB = 0, 
                   ineqUB = m, LB = c(0, 0), UB = c(Inf, Inf))
eqBarney1 <- solnp(c(1, 1), ux, ineqfun = bc, ineqLB = 0, 
                   ineqUB = m + sd*pd, LB = c(0, 0), UB = c(m/pb, Inf))
