...
dl.polynomial <- 
  ysym(deparse(polynomial)[[3]]) |> # Convert the "polynomial" function into a yac_sym. 
  deriv('x') |>                     # Take the derivative with respect to x. 
  yac_expr()                        # Convert the result into an r expression
yl.polynomial <- 
  (ysym(deparse(polynomial)[[3]]) / ysym('x')) |> # Divide converted polynomial by x.
  yac_expr()                                      # Convert the result into an r expression
...
```{r MPL-solution}
dl.polynomial <- ysym(deparse(polynomial)[[3]]) |>
  deriv('x') |>
  yac_expr()
yl.polynomial <- (ysym(deparse(polynomial)[[3]]) / ysym('x')) |>
  yac_expr()
polynomial.mpl <- function(x) {
  eval(dl.polynomial[[1]])
}
...
For the Cobb-Douglass example with $\alpha = 1/3$ and $A = 100$, plot the isoquant curves for $y = 1000$ and $y = 2500$ for $0 \le x_1 \le 50$ and $0 \le x_2 \le 50$.
