...
w =1; r = 2; alpha = 1/3; y0 = 2500
G <- ysym('L + 2*K + lambda*(2500 - 100*L^(1/3) * K^(2/3))')
(dG <- c(deriv(G, 'L'), deriv(G, 'K'), deriv(G, 'lambda')))
MPLinv <- solve(dG[[1]], 'lambda') |> y_rmvars()/w
MPLinv <- yac_str(MPLinv) |> Simplify() |> ysym()
MPKinv <- solve(dG[[2]], 'lambda') |> y_rmvars()/r
MPKinv <- yac_str(MPKinv) |> Simplify() |> ysym()
MRTS <- MPKinv/MPLinv
MRTSsolve <- solve(MRTS - w/r, 'K') |> y_rmvars() 
MRTSsolve <- gsub('\\}|\\{', '', MRTSsolve)
IQsub <- gsub('K', MRTSsolve, dG[[3]]) |> Simplify() |> ysym()
Lstar <- solve(IQsub, 'L') |> y_rmvars() |> yac_expr() |> eval()
Kstar <- parse(text = gsub('L', Lstar, MRTSsolve)) |> eval()
lambda <- parse(text = gsub('K', Kstar, gsub('L', Lstar, MPLinv))) |> eval()*w
...
Suppose that the firm wants to produce 25 units of output. Add the isoquant for the Cobb-Douglas production function from the previous section to the graph you started above. 
